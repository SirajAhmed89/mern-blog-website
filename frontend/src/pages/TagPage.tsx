    ...
    const fetchPosts = async () => {
      ...
      try {
        ...
        if (response.data?.data) {
          ...
          // Get tag name from first post or use slug
          if (response.data.data.length > 0) {
            const firstPost = response.data.data[0];
            const tags = firstPost.tags?.map(tag => 
              typeof tag === 'object' ? tag.name : tag
            ) || [];
            const matchingTag = tags.find(t => 
              t.toLowerCase().replace(/\s+/g, '-') === slug
            );
            setTagName(matchingTag || slug.replace(/-/g, ' '));
          } else {
      ...
      <div className="container mx-auto px-4">
        ...
        {posts.length > 0 ? (
          ...
          <div className="text-center py-16">
            <TagIcon className="w-16 h-16 mx-auto mb-4 text-neutral-400" />
